// Carrega os registros de treinamento do localStorage ou inicializa como um array vazio
const registrosTreinamento = JSON.parse(localStorage.getItem('registrosTreinamento')) || [];
...
function editar() {
    ...
    if (edicaoAtiva) {
        ...
        tabelaRelatorio.querySelectorAll('tr').forEach((linha, index) => {
            if (index < registrosTreinamento.length) {
    ...
    tabelaRelatorio.querySelectorAll('tr').forEach((linha, index) => {
        if (index < registrosTreinamento.length) {
            ...
            const dadosAtualizados = {
                ...
                tipoTreinamento: celulas[1].querySelector('input').value,
                dataRealizacao: celulas[2].querySelector('input').value
            };
            // Atualiza o registro no array de registrosTreinamento
            registrosTreinamento[index] = dadosAtualizados;
        }
    });
    ...
    switch (selectedFilter) {
        case 'diario': {
            ...
            filteredDates = dates.filter(registro => {
                const d = new Date(registro.dataRealizacao);
...
function baixarPDF() {
    ...
    doc.setFontSize(18);
    const title = 'Relatório de Realização de Treinamentos';
    ...
    const colunas = ['Funcionário', 'Treinamento', 'Data de Realização'];

    // Filtra os registros de acordo com a data
    let registros = filterDates(registrosTreinamento);

    // Prepara os dados para a tabela no PDF
    const dados = registrosTreinamento.map(dado => [
        dado.nomeFuncionario,
        dado.tipoTreinamento,
        dado.dataRealizacao
    ]);
    ...
    doc.save('relatorio-treinamentos.pdf');
}

// Função para preencher a tabela com os dados de registrosTreinamento
function preencherTabela() {
    tabelaRelatorio.innerHTML = '';
    if (registrosTreinamento.length > 0) {
        registrosTreinamento.forEach(dado => {
            const linha = tabelaRelatorio.insertRow();
            const celulaNome = linha.insertCell();
            celulaNome.textContent = dado.nomeFuncionario;

            const celulaTreinamento = linha.insertCell();
            celulaTreinamento.textContent = dado.tipoTreinamento;

            const celulaRealizacao = linha.insertCell();
            celulaRealizacao.textContent = dado.dataRealizacao;

            const celulaAcoes = linha.insertCell();
            // Adiciona um botão de excluir para cada linha
...
function excluirLinha(button) {
    ...
    registrosTreinamento.splice(index, 1);
    localStorage.setItem('registrosTreinamento', JSON.stringify(registrosTreinamento));
    row.remove();
}
